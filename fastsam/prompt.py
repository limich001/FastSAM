    ...
    def fast_show_mask(
        ...
        if random_color:
            ...
            if color_dict is not None:   
                color = np.ones((msak_sum, 1, 1, 3)) * np.array([int(color_dict["color"][2]) / 255, int(color_dict["color"][1]) / 255, int(color_dict["color"][0]) / 255])
            else:
        ...
        if random_color:
            ...
            if color_dict is not None:   
                color = torch.ones((msak_sum, 1, 1, 3)).to(annotation.device) * torch.tensor([
                    int(color_dict["color"][2]) / 255, int(color_dict["color"][1]) / 255, int(color_dict["color"][0]) / 255]).to(annotation.device)
            else:
                color = torch.ones((msak_sum, 1, 1, 3)).to(annotation.device) * torch.tensor([
